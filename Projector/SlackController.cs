            ...
            if (AppSettings.SlackEnabled)
            {
                ...
                p_Slack.OnEvent += (s, e) =>
                {
                    if (e.Data.Type == "message")
                    {
                        ...
                        if (message.Hidden)
                            // Slack sometimes sends Hidden messages. [Edits, deletes, and other updates.]
                        ...
                        if (message.Channel[0] == 'C') // Channels
                        {
                            if (AppSettings.SlackChannels.Contains(p_Slack.GetChannel(message.Channel).Name))
                            {
                                ...
                                text = Regex.Replace(text, @"\<\@(U[0-9A-Z]+)\>",
                                    (m) => "@" + p_Slack.GetUser(m.Groups[1].Value).Name);
                                text = Regex.Replace(text, @"\<\#(C[0-9A-Z]+)\>",
                                    (m) => "#" + p_Slack.GetChannel(m.Groups[1].Value).Name);
                        ...
                        else if (message.Channel[0] == 'D') // DMs.
                        {
                            ...
                            this.HandleCommand(userId, user, p_Slack.GetChannel(AppSettings.SlackChannels.First()).Id,
                ...
                switch (cmd)
                {
                    case "share":
                        switch (args.Length)
                        {
                            ...
                            case 2:
                                ...
                                {
                                    this.RespondPrimary(primaryChannelId,
                                        "Showing @" + args[1] + "'s screen on all projectors at @" + user +
                                        "'s request.");
                                }
                                else
                                {
                                ...
                                if (AppSettings.ProjectorName == args[2])
                                {
                                    if (this.m_Manager.SetSpecificClient(args[1]))
                                    {
                                        this.Respond(primaryChannelId,
                                            "Showing @" + args[1] + "'s screen on " + AppSettings.ProjectorName +
                                            " projector at @" + user +
                                            "'s request.");
                                    }
                                    else
                                    {
                                        this.Respond(imId,
                                            "There are no JamCast clients associated with the account name @" + args[1] +
                                            " on the " + AppSettings.ProjectorName + " projector");
                                    }
                                }
                                break;
                        }
                    ...
                    case "projectors":
                        ...
                        if (AppSettings.IsPrimary)
                        {
                            ...
                            {
                                this.Respond(imId,
                                    AppSettings.ProjectorName + " (primary projector, " + lockingInfo + ")");
                            }
                            else
                            {
                        ...
                        else
                        {
                            if (AppSettings.ProjectorName == args[2])
                            {
                                if (this.m_Manager.IsLocked)
                                {
                                    ...
                                    this.Respond(primaryChannelId,
                                        AppSettings.ProjectorName + " projector is now unlocked at @" + user +
                                        "'s request; projector " +
...
users [projector] - list all known users by this projector");
        ...
        {
            if (AppSettings.SlackEnabled && AppSettings.IsPrimary)
            {
